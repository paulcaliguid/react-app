...
import { getContact } from '../content/service'

export default function Contact() {
  const [data, setData] = useState<{ heading: string; intro?: string } | null>(null)
  useEffect(() => { getContact().then(setData).catch(() => {}) }, [])
  return (
    <section>
      <h2>{data?.heading ?? 'Contact'}</h2>
      {data?.intro && <p style={{ color: '#4b5563', maxWidth: 520 }}>{data.intro}</p>}
